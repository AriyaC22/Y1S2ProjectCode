...
MASS:float = 85E-3 #kg
MERR:float = 0.01E-3 #kg

AREA:float = np.pi*(DIAM**2)/4 #m^2
ARER:float = AREA * np.sqrt( (2 * DERR/DIAM)**2 ) #m^2
PRES:float = MASS*9.81/AREA + 101000#N m^-2
PERR:float = np.sqrt( (MASS/MERR)**2 + (2*ARER/ARER)**2 + (0.1)**2 )#N m^-2
...
def GetXY(gasDF:pd.DataFrame) -> pd.DataFrame:
    ...
    df = pd.DataFrame({
        ...
        "xerr": 2E-6,
        "yerr": 1/np.sqrt( (2 * gasDF['Freq']/gasDF['FreqError'])**2 * gasDF['Freq']**2 + (2 * gasDF['Lambda']/gasDF['LambdaError'])**2 * gasDF['Lambda']**2 ) #np.sqrt( ( (2 * gasDF['Freq'] * gasDF['FreqError'])/(gasDF['Freq']**2 + gasDF['Lambda']**-2)**2 )**2 + ( (2 * gasDF['Lambda']**-3 * gasDF['LambdaError'])/(gasDF['Freq']**2 + gasDF['Lambda']**-2)**2 ) )
    })
...
def Getgamma(C:float, cErr:float) -> tuple[float, float]:
    ...
    PerErr = np.sqrt( (MERR/MASS)**2 + (2*ARER/ARER)**2 + (PERR/PRES)**2 )
    print(f"%Err: {PerErr}")
    gamEr = gamma * np.sqrt( 0.05**2 + (cErr/C)**2 ) #np.sqrt( 0.05**2 + (cErr/C)**2 ) #np.sqrt( (MERR/MASS)**2 + (2*ARER/ARER)**2 + (PERR/PRES)**2 + (cErr/C)**2 )
    return gamma, gamEr
...
def PlotGas(title:str, dframe:pd.DataFrame, Fig, dicName:str=''):
    ...
    params, covariance = curve_fit(ModleEquation, dframe['x'], dframe['y'], sigma=dframe['yerr'], absolute_sigma=True)
    c, s = params
    ...
    Fig.errorbar(dframe['x'], dframe['y'], xerr=dframe['xerr'], yerr=dframe['yerr'], fmt='.', color='blue', zorder=1, linewidth=1)
...
def PlotGammaInfo():
    ...
    # try:print(f"Nitrogen mean:\n\t{np.mean([data2Calculate['Nitrogen Top'][4], data2Calculate['Nitrogen Bottom'][4]])}\nHelium mean:\n\t{np.mean([data2Calculate['Helium Top'][4], data2Calculate['Helium Bottom'][4]])}\n")
    # except: pass
